...
class AISDNController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Path for dashboard stats snapshot
        self.stats_path = "/tmp/ai_sdn/controller_stats.json"

        # Start background monitoring and snapshot threads
        self.monitor_thread = hub.spawn(self._monitor_flows)
        self.snapshot_thread = hub.spawn(self._snapshot_stats_loop)
    ...
    def _snapshot_stats_loop(self):
        """Periodically write controller stats for the CLI dashboard."""
        while True:
            hub.sleep(3)
            try:
                os.makedirs(os.path.dirname(self.stats_path), exist_ok=True)

                data = self.get_threat_stats()
                data["flow_stats_count"] = len(self.flow_stats)

                # Build very simple host zones from flows and blocked hosts
                host_zones = {"trusted": [], "monitored": [], "quarantine": []}
                for flow_key in list(self.flow_stats.keys()):
                    try:
                        src_ip, dst_ip, _ = flow_key.split("_", 2)
                    except ValueError:
                        continue

                    if src_ip in self.blocked_hosts:
                        if src_ip not in host_zones["quarantine"]:
                            host_zones["quarantine"].append(src_ip)
                    else:
                        if src_ip not in host_zones["monitored"]:
                            host_zones["monitored"].append(src_ip)

                    if dst_ip not in host_zones["trusted"]:
                        host_zones["trusted"].append(dst_ip)

                data["host_zones"] = host_zones

                with open(self.stats_path, "w") as f:
                    json.dump(data, f, indent=2)
            except Exception as e:
                self.logger.debug("Failed to write stats snapshot: %s", e)
